...
We begin by showing the resource use data for all \texttt{\Sexpr{nrow(diet.barcelona)}} pigeons in the sample from the Barcelona population, expressed as proportions (i.e., matrix $\mathbf{F}$ for Barcelona):
<<>>==
round(sweep(diet.barcelona, 1, FUN="/", 
            rowSums(diet.barcelona)), dig=3)
@
We see that most individuals in Barcelona feed on either sunflower or wheat, but there are some individuals (like pigeon 9) which prefer oats. Now we display the resource use data for the \texttt{\Sexpr{nrow(diet.moia)}} pigeons representing the population in Moià (i.e., matrix $\mathbf{F}$ for Moià):
<<>>==
