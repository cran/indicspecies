plotcoverage<-function(x, y=NULL, by=0.05, type="stat", max.order=NULL, group = NULL,  alpha = 0.05, add=FALSE, ...) {
   ...
   if(!add) {
     plot(Atseq,covA*100, type="s", ylim=c(0,100), axes=FALSE, xlab=expression(A[t]), ylab="Coverage (%)", ...)	
     axis(1, pos=0)
     axis(2, pos=0)     
