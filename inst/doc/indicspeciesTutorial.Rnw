...
\title{How to use the \texttt{indicspecies} package (ver. 1.6.7)}
...
@
As mentioned before, by default \texttt{multipatt} uses the IndVal index (\texttt{func = "IndVal.g"}) as test statistic. Actually, the square root of IndVal is returned by the \texttt{multipatt} function. The option \texttt{control = permControl(nperm=999)} allows choosing the number of random permutations required for the permutational test (this number affects the precision of the p-value). Function \texttt{permControl} from the \texttt{permute} package allows defining more complex permutational designs. 
...
sel
@
Object \texttt{sel} contains the indices of the \Sexpr{length(sel)} species that are candidates. Once we have our selection of candidate species, we can run the indicator analysis using:
<<>>=
